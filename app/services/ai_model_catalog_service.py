...
from app.services.ai_agent_service import FALLBACK_MODEL_OPTIONS, resolve_provider_program, safe_env_for_cli
...
class AiModelCatalogService:
    ...
    def load_catalog(self, provider_key: str, current_model: str = "") -> AiModelCatalog:
        payload = load_json(self.cache_path, {})
        if isinstance(payload, dict) and payload.get("version") == self.CACHE_VERSION:
            providers = payload.get("providers", {})
            raw_catalog = providers.get(provider_key) if isinstance(providers, dict) else None
    ...
    def fallback_catalog(self, provider_key: str, current_model: str = "") -> AiModelCatalog:
        ...
        catalog = AiModelCatalog(provider_key=provider_key, models=models, source="fallback")
        return self._with_current_model(catalog, current_model)
    ...
    def needs_refresh(
        ...
        if catalog.source not in {"live", "cache"} or not catalog.models:
            return True
        if cli_path and self._normalized_path(catalog.cli_path) != self._normalized_path(cli_path):
            return True
    ...
    def cli_identity(
        ...
        effective_deadline = deadline if deadline is not None else time.monotonic() + self.DISCOVERY_TIMEOUT_SECONDS
        return self._adapter_for(config.key).cli_identity(self, config, effective_deadline, cancel_event)
    ...
    def refresh(
        ...
        if not adapter.supports_live_discovery:
            return adapter.discover(self, config, program, cli_version, cancel_event, deadline)
        if current_catalog is not None and not force and not self.needs_refresh(
            current_catalog,
            program,
            cli_version,
        ):
            return self._with_current_model(current_catalog, config.model)
        catalog = adapter.discover(self, config, program, cli_version, cancel_event, deadline)
        catalog = self._validated_catalog(
            catalog,
    ...
    def _adapter_for(self, provider_key: str) -> ModelCatalogAdapter:
        try:
            ...
            raise ValueError(f"지원하지 않는 AI provider입니다: {provider_key}") from exc
    ...
    def save_catalog(self, catalog: AiModelCatalog) -> None:
        ...
        for model in catalog.models:
            if not isinstance(model, AiModelDescriptor):
                raise ValueError("AI model catalog contains an invalid model descriptor")
    ...
    def _discover_codex(
        ...
        try:
            ...
            self._wait_for_result(process, output_queue, request_id, effective_deadline, cancel_event)
            ...
            for page_index in range(self.MAX_PAGES):
                ...
                self._send(process, {"id": request_id, "method": "model/list", "params": params})
                ...
                if not isinstance(raw_models, list):
                    raise AiModelCatalogError("Codex 모델 목록 응답 형식이 올바르지 않습니다.")
                for raw_model in raw_models:
                    descriptor = self._descriptor_from_app_server(raw_model)
                    if descriptor.hidden:
                        continue
                    if descriptor.id in seen_ids:
                        raise AiModelCatalogError(f"Codex 모델 목록에 중복 항목(ID)이 있습니다: {descriptor.id}")
                    if descriptor.model in seen_models:
                        raise AiModelCatalogError(f"Codex 모델 목록에 중복 항목이 있습니다: {descriptor.model}")
                    seen_ids.add(descriptor.id)
                    seen_models.add(descriptor.model)
                    models.append(descriptor)
                    if len(models) > self.MAX_MODELS:
                        raise AiModelCatalogError("Codex 모델 목록이 허용 범위를 초과했습니다.")
                ...
                if not isinstance(next_cursor, str):
                    raise AiModelCatalogError("Codex 모델 목록 cursor 형식이 올바르지 않습니다.")
                if next_cursor in seen_cursors:
                    raise AiModelCatalogError("Codex 모델 목록 cursor가 반복되었습니다.")
                seen_cursors.add(next_cursor)
                cursor = next_cursor
                if page_index == self.MAX_PAGES - 1:
                    raise AiModelCatalogError("Codex 모델 목록 페이지 한도를 초과했습니다.")

            if not models:
                raise AiModelCatalogError("현재 계정에서 사용할 수 있는 Codex 모델을 찾지 못했습니다.")
            ...
            raise AiModelCatalogError(f"Codex 모델 목록 조회를 시작하지 못했습니다: {exc}") from exc
        finally:
    ...
    def _wait_for_result(
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                raise AiModelCatalogError("Codex app-server가 잘못된 JSON을 반환했습니다.") from exc
            if not isinstance(payload, dict):
                raise AiModelCatalogError("Codex app-server 응답 형식이 올바르지 않습니다.")
            ...
            if not isinstance(result, dict):
                raise AiModelCatalogError("Codex app-server 결과 형식이 올바르지 않습니다.")
            return result
    ...
    def _descriptor_from_app_server(raw_model: Any) -> AiModelDescriptor:
        ...
        if not all(isinstance(item, str) for item in (model_id, model_value, display_name, default_reasoning)):
            raise AiModelCatalogError("Codex 모델 문자열 필드 형식이 올바르지 않습니다.")
        if not isinstance(is_default, bool) or not isinstance(hidden, bool):
            raise AiModelCatalogError("Codex 모델 boolean 필드 형식이 올바르지 않습니다.")
        ...
        if not MODEL_VALUE_RE.fullmatch(model_id) or not MODEL_VALUE_RE.fullmatch(model_value):
            raise AiModelCatalogError("Codex 모델 ID 형식이 올바르지 않습니다.")
        ...
        for item in raw_reasoning:
            if not isinstance(item, dict):
                raise AiModelCatalogError("Codex 모델 추론 항목 형식이 올바르지 않습니다.")
            effort = item.get("reasoningEffort", "")
            if not isinstance(effort, str):
                raise AiModelCatalogError("Codex 모델 추론 단계 형식이 올바르지 않습니다.")
            effort = effort.strip()
            if effort not in AI_REASONING_EFFORTS:
                raise AiModelCatalogError(f"지원하지 않는 Codex 추론 단계입니다: {effort or '(없음)'}")
        ...
        if not isinstance(raw_speed, list) or any(not isinstance(item, str) for item in raw_speed):
        ...
        if availability is not None and not isinstance(availability, dict):
            raise AiModelCatalogError("Codex 모델 가용성 정보 형식이 올바르지 않습니다.")
        ...
        if not isinstance(availability_message, str):
            raise AiModelCatalogError("Codex 모델 가용성 메시지 형식이 올바르지 않습니다.")

        upgrade = raw_model.get("upgrade", "")
        if upgrade is None:
            upgrade = ""
        if not isinstance(upgrade, str):
            raise AiModelCatalogError("Codex 모델 업그레이드 정보 형식이 올바르지 않습니다.")
        ...
        if not upgrade and isinstance(upgrade_info, dict):
            ...
            if not isinstance(upgrade, str):
                raise AiModelCatalogError("Codex 모델 업그레이드 대상 형식이 올바르지 않습니다.")

        if not default_reasoning or default_reasoning not in reasoning:
            raise AiModelCatalogError("Codex 기본 추론 단계가 지원 목록과 일치하지 않습니다.")
        payload = {
            "id": model_id,
            "model": model_value,
    ...
    def _read_cli_version(
        ...
        effective_deadline = deadline if deadline is not None else time.monotonic() + self.DISCOVERY_TIMEOUT_SECONDS
        probe_deadline = min(effective_deadline, time.monotonic() + self.VERSION_TIMEOUT_SECONDS)
        process: subprocess.Popen[str] | None = None
        try:
            self._raise_if_cancelled(cancel_event)
            self._raise_if_deadline_expired(effective_deadline)
            ...
            while True:
                ...
                if process.returncode != 0:
                    return ""
                first_line = next((line.strip() for line in (stdout or "").splitlines() if line.strip()), "")
    ...
    def _validated_catalog(
        ...
        *,
        ...
        if catalog.provider_key not in KNOWN_AI_PROVIDERS or catalog.provider_key != expected_provider:
        ...
        for model in catalog.models:
            ...
            if not MODEL_VALUE_RE.fullmatch(normalized.id) or not MODEL_VALUE_RE.fullmatch(normalized.model):
            ...
            if len(normalized.availability_message) > 500:
                raise ValueError("AI model catalog contains oversized availability metadata")
            ...
            if normalized.id in seen_ids:
                raise ValueError(f"AI 모델 카탈로그에 중복 ID가 있습니다: {normalized.id}")
            if normalized.model in seen_models:
                raise ValueError(f"AI 모델 카탈로그에 중복 모델 값이 있습니다: {normalized.model}")
    ...
    @staticmethod
    def _with_current_model(catalog: AiModelCatalog, current_model: str) -> AiModelCatalog:
