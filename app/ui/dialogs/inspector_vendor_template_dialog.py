...
class PythonParserDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        actions = QHBoxLayout()
        test_button = make_action_button("테스트 실행", ActionKind.START)
...
class InspectorVendorTemplateDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        actions = QHBoxLayout()
        refresh_button = make_action_button("미리보기 갱신", ActionKind.REFRESH)
        refresh_button.clicked.connect(self.refresh_preview)
        self.save_button = make_action_button("템플릿 저장", ActionKind.SAVE)
    ...
    def _build_command_tab(self) -> None:
        ...
        buttons = QHBoxLayout()
        add_button = make_action_button("명령 추가", ActionKind.ADD)
        add_button.clicked.connect(self._add_command)
        remove_button = make_action_button("명령 삭제", ActionKind.DELETE)
    ...
    def _build_column_tab(self) -> None:
        ...
        buttons = QHBoxLayout()
        add_button = make_action_button("컬럼 추가", ActionKind.ADD)
        add_button.clicked.connect(self._add_column)
        remove_button = make_action_button("컬럼 삭제", ActionKind.DELETE)
    ...
    def _build_advanced_tab(self) -> None:
        ...
        layout.addWidget(hint)
        actions = QHBoxLayout()
        create_parser_button = make_action_button("Python 추출 함수 만들기", ActionKind.ADD)
        create_parser_button.clicked.connect(self._open_python_parser_dialog)
        open_parser_button = make_action_button("Python 추출 함수 폴더 열기", ActionKind.OPEN)
        open_parser_button.clicked.connect(lambda: os.startfile(str(self.service.custom_parsers_dir)))
        export_button = make_action_button("YAML 내보내기", ActionKind.EXPORT)
        export_button.clicked.connect(self._export_yaml)
        generate_button = make_action_button("기본 템플릿 파일 생성", ActionKind.ADD)
