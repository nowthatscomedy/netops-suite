...
class SwitchConfigBuilderWidget(QWidget):
    ...
    def _build_row_group(self) -> QGroupBox:
        group = QGroupBox("행 작업")
        layout = QVBoxLayout(group)
        self.add_row_button = make_action_button("행 추가", ActionKind.ADD)
        self.add_row_button.clicked.connect(self.add_row)
        self.duplicate_row_button = make_action_button("선택 행 복사", ActionKind.COPY)
        self.duplicate_row_button.clicked.connect(self.duplicate_selected_rows)
        self.increment_duplicate_row_button = make_action_button("연속 값 복사", ActionKind.COPY)
    ...
    def _build_pin_group(self) -> QGroupBox:
        ...
        self.show_all_columns_button = make_action_button("모두 표시", ActionKind.UTILITY)
        self.show_all_columns_button.clicked.connect(self.show_all_columns)
        self.clear_all_columns_button = make_action_button("모두 해제", ActionKind.CANCEL)
        ...
        self.column_move_up_button = make_action_button("위로", ActionKind.UTILITY)
        self.column_move_up_button.clicked.connect(lambda: self.move_selected_column_item(-1))
        self.column_move_down_button = make_action_button("아래로", ActionKind.UTILITY)
        self.column_move_down_button.clicked.connect(lambda: self.move_selected_column_item(1))
        self.delete_active_column_button = make_action_button("선택 컬럼 삭제", ActionKind.DELETE)
        ...
        self.column_pin_button = make_action_button("고정", ActionKind.UTILITY)
        self.column_pin_button.clicked.connect(self.pin_selected_column)
        self.column_unpin_button = make_action_button("고정 해제", ActionKind.UTILITY)
    ...
    def _build_file_group(self) -> QGroupBox:
        ...
        layout = QFormLayout(group)
        self.open_file_button = make_action_button("데이터 열기", ActionKind.BROWSE)
        self.open_file_button.clicked.connect(self.open_device_file_dialog)
        self.file_path_label = QLabel("-")
        self.file_path_label.setWordWrap(True)
        self.file_path_label.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Preferred)
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self.previous_device_button = make_action_button("이전", ActionKind.UTILITY)
        self.previous_device_button.clicked.connect(lambda: self._select_relative_visible_row(-1))
        self.next_device_button = make_action_button("다음", ActionKind.UTILITY)
        self.next_device_button.clicked.connect(lambda: self._select_relative_visible_row(1))
        self.copy_cli_button = make_action_button("복사", ActionKind.COPY)
        self.copy_cli_button.clicked.connect(self.copy_selected_cli)
        self.copy_next_cli_button = make_action_button("복사+다음", ActionKind.COPY)
