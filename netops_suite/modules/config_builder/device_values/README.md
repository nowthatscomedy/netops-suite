...
이 폴더의 CSV 파일은 Config Builder 프로필 작성법을 이해하기 위한 참고용 장비값입니다.
...
- `profile_id`는 사용할 YAML 프로필의 `id`와 같아야 합니다.
- `device_id`는 장비를 식별하기 위한 값입니다.
- 나머지 컬럼명은 YAML `variables`의 변수명과 맞춰 작성합니다.
- `sample_case`, `expected_result`, `note`처럼 프로필에 없는 컬럼은 참고 설명용으로 둘 수 있습니다.
